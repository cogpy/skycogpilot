...
@pytest.mark.asyncio
async def test_clean_finished_requests_with_retention_limit_functionality(
        isolated_database):
    """Test that the limit parameter controls batch size in cleanup operations."""
    current_time = time.time()
    retention_seconds = 60  # 1 minute retention
...
async def test_clean_finished_requests_with_retention_limit_larger_than_total(
        isolated_database):
    """Test cleanup when limit is larger than total number of old requests."""
    current_time = time.time()
...
@pytest.mark.asyncio
async def test_clean_finished_requests_with_retention_limit_one(
        isolated_database):
    """Test cleanup with limit=1 to verify single-request batching."""
    current_time = time.time()
    retention_seconds = 60
